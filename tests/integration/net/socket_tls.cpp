...
TEST(PlatformSecureSocket_Tls, TlsConnectionToPublicEndpoint)
{
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        size_t bytesRead = 0;
        sock.tryReceive(dummyBuffer.data(), dummyBuffer.size(), bytesRead);
    ...
    if (connected)
    {
        ...
        while (std::chrono::steady_clock::now() < deadline)
        {
            if (sock.tryReceive(receiveBuffer.data(), receiveBuffer.size(), bytesRead) && bytesRead > 0)
            {
                break;
            }
            std::this_thread::sleep_for(kPollSleepMs);
...
TEST(PlatformSecureSocket_Tls, HttpBinUuidEndpoint)
{
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        size_t bytesRead = 0;
        if (sock.tryReceive(buffer.data(), buffer.size(), bytesRead) && bytesRead > 0)
        {
