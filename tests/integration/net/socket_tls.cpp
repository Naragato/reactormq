...
{
    const auto settings = ConnectionSettingsBuilder().setHost("www.google.com").setPort(443).setShouldVerifyServerCertificate(false).
        build();
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        std::uint8_t dummyBuffer[64];
        size_t bytesRead = 0;
        sock.tryReceive(dummyBuffer, sizeof(dummyBuffer), bytesRead);
 ...
 "This may indicate cert verification issues or network problems.";
        ...
        const bool sendSuccess = sock.trySend(
            reinterpret_cast<const std::uint8_t*>(httpRequest), static_cast<std::uint32_t>(std::strlen(httpRequest)), bytesSent);

        EXPECT_TRUE(sendSuccess) << "Failed to send HTTP request over TLS";
        EXPECT_GT(bytesSent, 0) << "No bytes sent over TLS connection";

        std::uint8_t receiveBuffer[1024];
        size_t bytesRead = 0;
        while (std::chrono::steady_clock::now() < deadline)
        {
            if (sock.tryReceive(receiveBuffer, sizeof(receiveBuffer), bytesRead) && bytesRead > 0)
            {
                break;
            }
            std::this_thread::sleep_for(kPollSleepMs);
...
TEST(PlatformSecureSocket_Tls, HttpBinUuidEndpoint)
{
    ...
    const auto httpRequest = "GET /uuid HTTP/1.1\r\n" "Host: httpbin.org\r\n" "Connection: close\r\n" "\r\n";
...
TEST(PlatformSecureSocket_Tls, ExampleDotComKnownContent)
{
    ...
    const auto httpRequest = "GET / HTTP/1.1\r\n" "Host: example.com\r\n" "Connection: close\r\n" "\r\n";
...
{
    const auto settings = ConnectionSettingsBuilder().setHost("expired.badssl.com").setPort(443).setShouldVerifyServerCertificate(true).
        build();
    PlatformSecureSocket sock(settings);
...
{
    const auto settings = ConnectionSettingsBuilder().setHost("self-signed.badssl.com").setPort(443).setShouldVerifyServerCertificate(false)
        .build();
    PlatformSecureSocket sock(settings);
...
TEST(PlatformSecureSocket_Tls, HttpBinBytesEndpointLargeTransfer)
{
    ...
    const auto httpRequest = "GET /bytes/8192 HTTP/1.1\r\n" "Host: httpbin.org\r\n" "Connection: close\r\n" "\r\n";
