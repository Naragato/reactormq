...
namespace
{
    ...
    void setNonBlocking(const SocketHandle s)
    {
    ...
    {
    public:
        explicit SocketGuard(const SocketHandle sock)
            : m_sock(sock)
        {
        }
        ~SocketGuard()
    ...
    private:
        SocketHandle m_sock{ InvalidSocket };
    };
    ...
    bool probeMqttBroker(const SocketHandle sock, const unsigned hostPort)
    {
...
namespace reactormq::test
{
    std::string DockerContainer::trim(std::string s)
    {
        ...
        REACTORMQ_LOG(
            ...
            "[trim] original_size=%zu trimmed_size=%zu",
    ...
    bool DockerContainer::waitForRunning(const std::chrono::seconds timeoutSeconds) const
    {
        REACTORMQ_LOG(
            ...
            "[waitForRunning] name=\"%s\" timeout=%llds",
            m_name.c_str(),
    ...
    bool DockerContainer::waitForPort(const std::chrono::seconds timeoutSeconds) const
    {
        REACTORMQ_LOG(
            logging::LogLevel::Info,
            "[waitForPort] hostPort=%u timeout=%llds",
            m_hostPort,
            timeoutSeconds.count());
