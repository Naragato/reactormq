...
namespace reactormq::test
{
    namespace detail
    {
        ...
        {
#ifdef _WIN32
            if (s != kRmqInvalidSocket) { closesocket(s); }
#else
            if (s != kRmqInvalidSocket) { ::close(s); }
#endif // _WIN32
        }
    }
     ...
     */
    inline std::optional<std::uint16_t> findAvailablePort(const std::uint16_t startPort,
                                                          const std::uint16_t endPort)
    {
        ...
        {
            RmqSocketHandle s = ::socket(AF_INET, SOCK_STREAM, 0);
