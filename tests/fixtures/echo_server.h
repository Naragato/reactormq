...
namespace reactormq::tests
{
...
#ifdef _WIN32
            setsockopt(m_listenSocket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&enable), sizeof(enable));
#else
    ...
    private:
        ...
        static void echoLoop(const SocketHandle client)
        {
            ...
            {
#ifdef _WIN32
                const int received = ::recv(client, reinterpret_cast<char *>(buffer.data()), static_cast<int>(buffer.size()),
                                      0);
#else
                const size_t received = ::recv(client, buffer.data(), buffer.size(), 0);
#endif // _WIN32
                if (received <= 0)
                {
                    break;
...
#else
                    const size_t sent = ::send(client, outPtr, static_cast<size_t>(remaining), 0);
#endif // _WIN32
                    if (sent <= 0)
                    {
                        return;
