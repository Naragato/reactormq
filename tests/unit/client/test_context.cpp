...
TEST(ContextTest, InvokeCallbackUsesExecutorWhenProvided)
{
    ...
    ConnectionSettingsBuilder b;
    b.setHost("localhost").setCallbackExecutor(
        [&usedExecutor](auto cb)
        {
            usedExecutor = true;
            cb();
        });
...
TEST(ContextTest, GetPendingCommandCountCountsPublishesSubscribesUnsubscribes)
{
    ...
    ctx.storePendingPublish(
        1, PublishCommand{ Message{ "t", Message::Payload{}, false, QualityOfService::AtMostOnce }, std::promise<Result<void>>{} });
    ctx.storePendingSubscribe(
        2, SubscribeCommand{ TopicFilter{ "a", QualityOfService::AtLeastOnce }, std::promise<Result<SubscribeResult>>{} });
    ctx.storePendingSubscribes(
        3, SubscribesCommand{ { TopicFilter{ "b", QualityOfService::AtLeastOnce } },
                              std::promise<Result<std::vector<SubscribeResult>>>{} });
...
TEST(ContextTest, ParsePacketReturnsNullptrOnInvalidData)
{
    ...
    constexpr std::uint8_t buf1[1]{};
    EXPECT_EQ(ctx.parsePacket(buf1, 1), nullptr);
}
...
TEST(ContextTest, ParsePacketReturnsNullptrWhenExceedsMaxPacketSize)
{
    ...
    Context ctx(b.build());
    constexpr std::uint8_t buf[8]{};
    EXPECT_EQ(ctx.parsePacket(buf, 8), nullptr);
}
...
TEST(ContextTest, ParsePacketParsesConnectPacketForV311)
{
    ...
    const packets::Connect<packets::ProtocolVersion::V311> connect(
        "client", 60, "", "", true, false, "", "", QualityOfService::AtMostOnce);
    connect.encode(writer);

    const auto pkt = ctx.parsePacket(reinterpret_cast<const std::uint8_t*>(buffer.data()), static_cast<std::uint32_t>(buffer.size()));
