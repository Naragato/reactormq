...
namespace
{
    class FakeSocket final : public SocketBase
    {
    public:
        explicit FakeSocket(ConnectionSettingsPtr settings)
            : SocketBase(std::move(settings))
        {
        }
...
TEST(ReactorTest, SetupSocketCallbacksOnlyOnce)
{
    Reactor r(makeSettings());
    ...
    EXPECT_EQ(fake->getOnConnectCallback().getSize(), 0u);
    EXPECT_EQ(fake->getOnDisconnectCallback().getSize(), 0u);
    EXPECT_EQ(fake->getOnDataReceivedCallback().getSize(), 0u);

    r.tick();
    EXPECT_EQ(fake->getOnConnectCallback().getSize(), 1u);
    EXPECT_EQ(fake->getOnDisconnectCallback().getSize(), 1u);
    EXPECT_EQ(fake->getOnDataReceivedCallback().getSize(), 1u);
