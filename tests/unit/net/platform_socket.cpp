...
TEST(PlatformSocket, SendAndReceive)
{
    ...
    for (int i = 0; i < 100; ++i)
    {
        if (socket.tryReceive(receiveBuffer.data(), static_cast<int32_t>(receiveBuffer.size()), bytesRead))
        {
            if (bytesRead > 0)
            {
                break;
...
TEST(PlatformSocket, MultipleMessages)
{
    ...
    for (int i = 0; i < 100 && totalBytesRead < expectedTotal; ++i)
    {
        ...
        if (socket.tryReceive(
            receiveBuffer.data() + totalBytesRead, static_cast<int32_t>(receiveBuffer.size() - totalBytesRead), bytesRead))
        {
            if (bytesRead > 0)
            {
...
TEST(PlatformSocket, GetPendingData)
{
    ...
    for (int i = 0; i < 50; ++i)
    {
        const size_t pending = socket.getPendingData();
        if (pending > 0)
        {
...
TEST(PlatformSocket, ReceiveWithPeekFlag)
{
    ...
    for (int i = 0; i < 100; ++i)
    {
        if (socket.tryReceiveWithFlags(peekBuffer.data(), static_cast<int32_t>(peekBuffer.size()), ReceiveFlags::Peek, bytesRead))
        {
            if (bytesRead > 0)
            {
                break;
...
TEST(PlatformSocket, LargeDataTransfer)
{
    ...
    {
        size_t bytesSent = 0;
        const auto remaining = static_cast<uint32_t>(largeSize - totalBytesSent);
        if (socket.trySend(largeData.data() + totalBytesSent, remaining, bytesSent))
        {
            if (bytesSent > 0)
            {
