...
TEST(NativeSocket_MqttFraming, SingleCompletePacket)
{
    ...
    const auto settings = ConnectionSettingsBuilder{}.setHost("127.0.0.1").setPort(port).setProtocol(ConnectionProtocol::Tcp).
...
TEST(NativeSocket_MqttFraming, TwoConsecutivePackets)
{
    ...
    auto settings = ConnectionSettingsBuilder{}.setHost("127.0.0.1").setPort(port).setProtocol(ConnectionProtocol::Tcp).
        setCredentialsProvider(std::make_shared<NoOpCredentialsProvider>()).build();
