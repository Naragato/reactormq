...
{
    const auto prop = Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(1);
...
{
    const auto prop = Property::create<PropertyIdentifier::ServerKeepAlive, uint16_t>(300);
...
{
    const auto prop = Property::create<PropertyIdentifier::MessageExpiryInterval, uint32_t>(3600);
...
{
    const auto prop = Property::create<PropertyIdentifier::SubscriptionIdentifier, uint32_t>(128);
...
{
    const auto prop = Property::create<PropertyIdentifier::ContentType, std::string>("application/json");
...
TEST(Property, CreateAndEncode_Binary_CorrelationData)
{
    ...
    const auto prop = Property::create<PropertyIdentifier::CorrelationData, std::vector<uint8_t>>(data);
...
{
    const auto prop = Property::create<PropertyIdentifier::UserProperty, std::pair<std::string, std::string>>(
...
{
    const std::vector buffer = { std::byte{ 3 },
                                 std::byte{ 0x00 },
                                 std::byte{ 0x05 },
                                 std::byte{ 'h' },
                                 std::byte{ 'e' },
...
{
    const std::vector buffer = { std::byte{ 9 },
                                 std::byte{ 0x00 },
                                 std::byte{ 0x03 },
                                 std::byte{ 0x01 },
                                 std::byte{ 0x02 },
...
{
    const std::vector buffer = { std::byte{ 38 },
                                 std::byte{ 0x00 },
                                 std::byte{ 0x03 },
                                 std::byte{ 'k' },
                                 std::byte{ 'e' },
...
{
    constexpr PropertyIdentifier uint8Props[] = { PropertyIdentifier::PayloadFormatIndicator,
    ...
    {
        Property prop = id == PropertyIdentifier::PayloadFormatIndicator
            ? Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(42)
            : id == PropertyIdentifier::RequestProblemInformation
            ? Property::create<PropertyIdentifier::RequestProblemInformation, uint8_t>(42)
            : id == PropertyIdentifier::RequestResponseInformation
            ? Property::create<PropertyIdentifier::RequestResponseInformation, uint8_t>(42)
            : id == PropertyIdentifier::MaximumQoS
            ? Property::create<PropertyIdentifier::MaximumQoS, uint8_t>(42)
            : id == PropertyIdentifier::RetainAvailable
            ? Property::create<PropertyIdentifier::RetainAvailable, uint8_t>(42)
            : id == PropertyIdentifier::WildcardSubscriptionAvailable
            ? Property::create<PropertyIdentifier::WildcardSubscriptionAvailable, uint8_t>(42)
            : id == PropertyIdentifier::SubscriptionIdentifierAvailable
            ? Property::create<PropertyIdentifier::SubscriptionIdentifierAvailable, uint8_t>(42)
            : Property::create<PropertyIdentifier::SharedSubscriptionAvailable, uint8_t>(42);
...
{
    {
        const auto prop = Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(255);
        EXPECT_EQ(prop.toString(), "255");
    }

    {
        const auto prop = Property::create<PropertyIdentifier::ServerKeepAlive, uint16_t>(1234);
        EXPECT_EQ(prop.toString(), "1234");
    }

    {
        const auto prop = Property::create<PropertyIdentifier::MessageExpiryInterval, uint32_t>(123456);
        EXPECT_EQ(prop.toString(), "123456");
    }

    {
        const auto prop = Property::create<PropertyIdentifier::ContentType, std::string>("text/plain");
...
{
    const auto prop1 = Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(1);
    const auto prop2 = Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(1);
    const auto prop3 = Property::create<PropertyIdentifier::PayloadFormatIndicator, uint8_t>(2);
    const auto prop4 = Property::create<PropertyIdentifier::RequestProblemInformation, uint8_t>(1);
...
{
    const auto msg = Property::create<PropertyIdentifier::MessageExpiryInterval, uint32_t>(1000);
    const auto sub = Property::create<PropertyIdentifier::SubscriptionIdentifier, uint32_t>(2000);
    const auto ses = Property::create<PropertyIdentifier::SessionExpiryInterval, uint32_t>(3000);
    const auto wil = Property::create<PropertyIdentifier::WillDelayInterval, uint32_t>(4000);
    const auto max = Property::create<PropertyIdentifier::MaximumPacketSize, uint32_t>(5000);
...
{
    const auto ct = Property::create<PropertyIdentifier::ContentType, std::string>("ct");
    const auto rt = Property::create<PropertyIdentifier::ResponseTopic, std::string>("rt");
    const auto ac = Property::create<PropertyIdentifier::AssignedClientIdentifier, std::string>("ac");
    const auto am = Property::create<PropertyIdentifier::AuthenticationMethod, std::string>("am");
    const auto ri = Property::create<PropertyIdentifier::ResponseInformation, std::string>("ri");
    const auto sr = Property::create<PropertyIdentifier::ServerReference, std::string>("sr");
    const auto rs = Property::create<PropertyIdentifier::ReasonString, std::string>("rs");
...
{
    const auto sk = Property::create<PropertyIdentifier::ServerKeepAlive, uint16_t>(100);
    const auto rm = Property::create<PropertyIdentifier::ReceiveMaximum, uint16_t>(200);
    const auto tam = Property::create<PropertyIdentifier::TopicAliasMaximum, uint16_t>(300);
    const auto ta = Property::create<PropertyIdentifier::TopicAlias, uint16_t>(400);
...
TEST(Property, BinaryProperties)
{
    ...
    const auto cd = Property::create<PropertyIdentifier::CorrelationData, std::vector<uint8_t>>(data1);
    const auto ad = Property::create<PropertyIdentifier::AuthenticationData, std::vector<uint8_t>>(data2);
