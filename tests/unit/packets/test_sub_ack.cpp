...
TEST(SubAck3, Constructor_SingleReturnCode)
{
    ...
    EXPECT_EQ(packet.getReturnCodes().size(), 1u);
    EXPECT_EQ(packet.getReturnCodes()[0], SubscribeReturnCode::SuccessQualityOfService0);
}

TEST(SubAck3, Constructor_MultipleReturnCodes)
{
    const std::vector returnCodes = { SubscribeReturnCode::SuccessQualityOfService0,
                                      SubscribeReturnCode::SuccessQualityOfService1,
                                      SubscribeReturnCode::SuccessQualityOfService2,
                                      SubscribeReturnCode::Failure };
    const SubAck3 packet(100, returnCodes);

    EXPECT_EQ(packet.getPacketId(), 100u);
    EXPECT_EQ(packet.getLength(), 6u);
    EXPECT_EQ(packet.getReturnCodes().size(), 4u);
}
...
TEST(SubAck3, Decode_SingleReturnCode)
{
    ...
    EXPECT_EQ(packet.getReturnCodes().size(), 1u);
    EXPECT_EQ(packet.getReturnCodes()[0], SubscribeReturnCode::SuccessQualityOfService1);
}
...
TEST(SubAck3, Decode_MultipleReturnCodes)
{
    ...
    EXPECT_EQ(packet.getReturnCodes().size(), 4u);
    EXPECT_EQ(packet.getReturnCodes()[0], SubscribeReturnCode::SuccessQualityOfService0);
    EXPECT_EQ(packet.getReturnCodes()[1], SubscribeReturnCode::SuccessQualityOfService1);
    EXPECT_EQ(packet.getReturnCodes()[2], SubscribeReturnCode::SuccessQualityOfService2);
    EXPECT_EQ(packet.getReturnCodes()[3], SubscribeReturnCode::Failure);
}
...
TEST(SubAck3, RoundTrip_EncodeAndDecode)
{
    ...
    EXPECT_EQ(packet2.getReturnCodes().size(), packet1.getReturnCodes().size());
    for (size_t i = 0; i < packet1.getReturnCodes().size(); ++i)
    {
        EXPECT_EQ(packet2.getReturnCodes()[i], packet1.getReturnCodes()[i]);
    }
}

TEST(SubAck3, GetLength_Varies)
{
    const std::vector codes1 = { SubscribeReturnCode::SuccessQualityOfService0 };
    const SubAck3 packet1(1, codes1);
    EXPECT_EQ(packet1.getLength(), 3u);

    const std::vector codes2 = { SubscribeReturnCode::SuccessQualityOfService0,
                                 SubscribeReturnCode::SuccessQualityOfService1,
                                 SubscribeReturnCode::SuccessQualityOfService2 };
...
TEST(SubAck5, Constructor_WithProperties)
{
    ...
    const SubAck5 packet(42, reasonCodes, props);
...
{
    const std::vector reasonCodes = { ReasonCode::Success, ReasonCode::GrantedQualityOfService2, ReasonCode::NotAuthorized };
    const SubAck5 packet(42, reasonCodes);
...
{
    const std::vector codes1 = { ReasonCode::Success };
    const SubAck5 packet1(1, codes1);
    EXPECT_EQ(packet1.getLength(), 4u);

    const std::vector codes2 = { ReasonCode::Success, ReasonCode::GrantedQualityOfService1, ReasonCode::GrantedQualityOfService2 };
