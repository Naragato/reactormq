...
TEST(Disconnect5, Constructor_WithProperties)
{
    ...
    const Disconnect5 packet(ReasonCode::ServerShuttingDown, props);
...
TEST(Disconnect5, IsReasonCodeValid_ValidCodes)
{
    ...
    {
        EXPECT_TRUE(Disconnect5::isReasonCodeValid(code))
...
TEST(Disconnect5, IsReasonCodeValid_InvalidCodes)
{
    ...
    {
        EXPECT_FALSE(Disconnect5::isReasonCodeValid(code))
...
TEST(Disconnect5Properties, EncodeDecode_UserProperty)
{
    ...
    const Disconnect5 packet(ReasonCode::ServerBusy, props);
