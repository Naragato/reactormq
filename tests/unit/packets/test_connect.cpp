...
TEST(Connect5AuthProps, EncodeDecode_AuthenticationMethodAndData)
{
    ...
    propList.emplace_back(
        Property::create<PropertyIdentifier::AuthenticationData, std::vector<uint8_t>>(std::vector<uint8_t>{ 1, 2, 3, 4 }));
    Properties connectProps(propList);

    Connect5 packet(
        clientId, keepAlive, username, password, false, false, std::string{}, std::string{}, QualityOfService::AtMostOnce, Properties{},
        connectProps);
