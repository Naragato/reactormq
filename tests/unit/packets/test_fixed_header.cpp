...
public:
    MockControlPacket(
        const PacketType type, const uint32_t length, const uint16_t packetId = 0,
        const bool valid = true)
        : m_type(type), m_length(length), m_packetId(packetId), m_valid(valid)
    {
    }
...
TEST(FixedHeader, CreateWithCustomFlags_NoFlagsSet)
{
    ...
    const FixedHeader header = FixedHeader::create(
        &packet,
        100,
        false,
...
TEST(FixedHeader, RoundTrip_PublishWithFlags)
{
    ...
    const FixedHeader header1 = FixedHeader::create(
        &packet,
        256,
        true,
        QualityOfService::ExactlyOnce,
