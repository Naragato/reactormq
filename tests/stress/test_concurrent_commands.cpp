...
{
    auto settings = std::make_shared<ConnectionSettings>(
        "127.0.0.1",
        m_port,
    ...
    constexpr std::size_t kCommandsPerThread = 1000;
    ...
    {
        threads.emplace_back([&reactor, &enqueuedCount]()
        {
            for (std::size_t j = 0; j < kCommandsPerThread; ++j)
            {
                ConnectCommand cmd;
    ...
    std::thread processorThread([&reactor, &enqueuedCount]()
    {
