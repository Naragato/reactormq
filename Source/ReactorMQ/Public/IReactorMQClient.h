...
public:
	...
	virtual TFuture<TReactorMQResult<void>> PublishAsync(const FString& Topic, const TArray<uint8>& Payload, EReactorMQQualityOfService QoS, bool bRetain) = 0;
	virtual TFuture<TReactorMQResult<TArray<FReactorMQSubscribeResult>>> SubscribeAsync(const TArray<FReactorMQTopicFilter>& TopicFilters) = 0;
