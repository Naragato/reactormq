...
namespace reactormq::mqtt
{
    ...
    public:
        ...
        add(T* obj, F&& lambda)
        {
            ...
            static_assert(
                std::is_invocable_r_v<R, F&, Args...>,
                "Lambda must be callable with Args... and return R");
     ...
     */
    using OnUnsubscribe = MulticastDelegate<void
        (const std::shared_ptr<std::vector<UnsubscribeResult>>& resultsPerTopic)>;
} // namespace reactormq::mqtt