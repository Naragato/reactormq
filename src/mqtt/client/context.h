...
namespace reactormq::mqtt::packets
{
    ...
    template<ProtocolVersion V> class Connect;
    template<ProtocolVersion V> class ConnAck;
    template<ProtocolVersion V> class Publish;
    template<ProtocolVersion V> class PubAck;
    template<ProtocolVersion V> class PubRec;
    template<ProtocolVersion V> class PubRel;
    template<ProtocolVersion V> class PubComp;
    template<ProtocolVersion V> class Subscribe;
    template<ProtocolVersion V> class SubAck;
    template<ProtocolVersion V> class Unsubscribe;
    template<ProtocolVersion V> class UnsubAck;
    template<ProtocolVersion V> class Disconnect;
}
...
namespace reactormq::mqtt::client
{
    ...
    public:
         ...
         */
        [[nodiscard]] bool canAddToOutboundQueue(const std::size_t bytes) const
        {
            if (!m_settings)
            {
                return true;
        ...
        void addOutboundQueueSize(const std::size_t bytes)
        {
            m_outboundQueueSize += bytes;
        }

        /// @brief Decrease the outbound queue size by the given bytes. 
        void subtractOutboundQueueSize(const std::size_t bytes)
        {
        ...
        void encodePublishForCurrentVersion(
            Message const& message,
            std::uint16_t packetId,
            serialize::ByteWriter& writer) const;

        /// @brief Retransmit pending QoS 1/2 publishes with DUP set (on reconnect).
        void retransmitPendingPublishes();

        std::vector<std::byte> encodePendingPublish(
            PublishCommand const& publishCmd,
    ...
    private:
        ...
        std::unordered_map<std::uint16_t, SubscribeCommand> m_pendingSubscribes; ///< Map for tracking pending subscribes
        std::unordered_map<std::uint16_t, SubscribesCommand> m_pendingSubscribesMulti; ///< Map for tracking pending multi-topic subscribes
        std::unordered_map<std::uint16_t, UnsubscribesCommand> m_pendingUnsubscribes; ///< Map for tracking pending unsubscribes
