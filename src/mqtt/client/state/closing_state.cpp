...
namespace reactormq::mqtt::client
{
    namespace
    {
        template<packets::ProtocolVersion V> void encodeDisconnect(serialize::ByteWriter& writer)
        {
            using Mapping = MqttVersionMapping<V>;
            if constexpr (V == packets::ProtocolVersion::V5)
            {
                const typename Mapping::Disconnect disconnectPacket(ReasonCode::NormalDisconnection, {});
                disconnectPacket.encode(writer);
            }
            else
            {
                const typename Mapping::Disconnect disconnectPacket;
                disconnectPacket.encode(writer);
            }
        }
    }
    ...
    StateTransition ClosingState::onEnter(Context& context)
    {
        ...
        if (sock)
        {
            ...
            withMqttVersion(
                context.getProtocolVersion(), [&writer]<typename VersionTag>(VersionTag)
                {
                    constexpr auto kV = VersionTag::value;
                    encodeDisconnect<kV>(writer);
                });
