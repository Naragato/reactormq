...
namespace reactormq::mqtt::client
{
    ...
    std::unique_ptr<packets::IControlPacket> Context::parsePacket(const std::span<const std::byte> data) const
    {
        if (data.size() < 2)
        {
            REACTORMQ_LOG(reactormq::logging::LogLevel::Error, "Failed to parse packet: insufficient data (size: %u)", data.size());
            return nullptr;
        }
        ...
        if (m_settings && m_settings->shouldEnforceMaxPacketSize())
        {
            ...
            {
                REACTORMQ_LOG(
                    reactormq::logging::LogLevel::Error, "Packet size (%u bytes) exceeds maximum allowed (%u bytes)", data.size(),
                    maxPacketSize);
                return nullptr;
            }
        }
        ...
        REACTORMQ_LOG(
            reactormq::logging::LogLevel::Debug, "Received packet - Type: %d, PacketId: %u", static_cast<int>(result->getPacketType()),
            result->getPacketId());
    ...
    template<typename VersionTag, typename Message> void Context::encodePublish(
        Message const& message, std::uint16_t packetId, serialize::ByteWriter& writer) const
    {
        constexpr auto kV = VersionTag::value;
    ...
    {
        withMqttVersion(
            m_protocolVersion, [&message, &packetId, &writer, this]<typename VersionTag>(VersionTag)
            {
