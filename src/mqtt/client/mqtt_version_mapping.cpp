...
{
    template<packets::ProtocolVersion V> std::unique_ptr<packets::IControlPacket> parsePacketImpl(
        ...
        Message const& message, const std::uint16_t packetId, serialize::ByteWriter& writer)
    {
        ...
        Mapping::Publish publishPacket(
            message.getTopic(), message.getPayload(), message.getQualityOfService(), message.shouldRetain(), packetId, true // dup
            );
        ...
        Message const& message, const std::uint16_t packetId, serialize::ByteWriter& writer)
    {
        ...
        Mapping::Publish publishPacket(
            message.getTopic(), message.getPayload(), message.getQualityOfService(), message.shouldRetain(), packetId, {}, // properties
            true // dup
            );
