...
namespace reactormq::mqtt::packets
{
    ...
    bool Auth::decode(serialize::ByteReader& reader)
    {
        ...
        if (m_fixedHeader.getPacketType() != PacketType::Auth)
        {
            REACTORMQ_LOG(
                logging::LogLevel::Error, "[Auth] Invalid packet type: expected Auth, got %d",
        ...
        {
            REACTORMQ_LOG(logging::LogLevel::Error, "[Auth] Invalid reason code: %u", static_cast<uint8_t>(m_reasonCode));
            return false;
        }
    ...
    bool Auth::arePropertiesValid(const properties::Properties& properties)
    {
        for (const auto props = properties.getProperties(); const auto& prop : props)
        {
            ...
            {
            case properties::PropertyIdentifier::AuthenticationMethod:
            case properties::PropertyIdentifier::AuthenticationData:
            case properties::PropertyIdentifier::ReasonString:
            case properties::PropertyIdentifier::UserProperty:
                break;
            default: REACTORMQ_LOG(logging::LogLevel::Error, "[Auth] Invalid property identifier: %u", static_cast<uint8_t>(id));
                return false;
            }
        }
