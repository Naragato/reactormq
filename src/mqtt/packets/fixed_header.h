...
namespace reactormq::mqtt::packets
{
    ...
    public:
        ...
        [[nodiscard]] PacketType getPacketType() const
        {
            ...
            return static_cast<PacketType>(
                std::to_integer<uint8_t>((flags & kTypeMask) >> kTypeShift));
        }
            ...
            const bool isDuplicated)
        {
            ...
            const auto qosBits =
                    (static_cast<std::byte>(static_cast<uint8_t>(qos)) & kQosMask) << kQosShift;
            flags |= qosBits;
        ...
        void decode(serialize::ByteReader& reader)
        {
            ...
            constexpr auto maxAllowedType = static_cast<uint8_t>(PacketType::Auth);
    ...
    private:
        ...
        FixedHeader(const uint8_t flags, const uint32_t remainingLength)
            : m_flags(flags), m_remainingLength(remainingLength)
        {
        }
