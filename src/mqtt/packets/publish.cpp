    ...
    bool Publish<ProtocolVersion::V311>::decode(serialize::ByteReader& reader)
    {
        ...
        if (m_fixedHeader.getPacketType() != PacketType::Publish)
        {
            REACTORMQ_LOG(
                ...
                "Publish",
                "[Publish] Invalid packet type: expected Publish, got %d",
        ...
        if (static_cast<size_t>(payloadSize) > reader.getRemaining())
        {
            REACTORMQ_LOG(
                logging::LogLevel::Error,
                "Publish",
        ...
        m_payload.resize(static_cast<size_t>(payloadSize));
        reader.readBytes(reinterpret_cast<std::byte*>(m_payload.data()), static_cast<size_t>(payloadSize));
