...
namespace reactormq::mqtt::packets::properties
{
                ...
                std::conditional_t<
                    ...
                    std::conditional_t<
                        ...
                        std::conditional_t<
                            ...
                            std::vector<uint8_t>,
                            std::conditional_t<
                                Id == PropertyIdentifier::UserProperty,
    ...
    concept PropertyValueFor =
            std::same_as<std::decay_t<T>, property_value_type_t<Id> >
            || (std::same_as<property_value_type_t<Id>, std::string> && std::convertible_to<T, std::string>);

    bool operator==(
        const std::variant<std::monostate, uint8_t, uint16_t, uint32_t, std::vector<uint8_t>, std::string, std::pair<
            std::string, std::string> >& lhs,
        const std::variant<std::monostate, uint8_t, uint16_t, uint32_t, std::vector<uint8_t>, std::string, std::pair<
    ...
    {
    private:
        using PropertyData = std::variant<
            std::monostate,
            uint8_t,
            uint16_t,
            uint32_t,
    ...
    public:
        static constexpr const char* kInvalidPropertyIdentifier = "Invalid property identifier";
        static constexpr const char* kFailedToReadPropertyError = "Failed to read property.";
         ...
         */
        [[nodiscard]] PropertyIdentifier getIdentifier() const { return m_identifier; }
