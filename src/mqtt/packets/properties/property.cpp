...
namespace reactormq::mqtt::packets::properties
{
    ...
    {
        template<class T, class U> bool operator()(const T&, const U&) const noexcept
        {
            return false;
        }

        template<class T> bool operator()(const T& a, const T& b) const noexcept(noexcept(a == b))
        {
            return a == b;
        }
    };
    ...
    bool operator==(
        const std::variant<std::monostate, uint8_t, uint16_t, uint32_t, std::vector<uint8_t>, std::string, std::pair<std::string,
            std::string>>& lhs,
        const std::variant<std::monostate, uint8_t, uint16_t, uint32_t, std::vector<uint8_t>, std::string, std::pair<std::string,
            std::string>>& rhs)
    {
    ...
    void Property::encode(ByteWriter& w) const
    {
        ...
        case Max:
        case Unknown: default: REACTORMQ_LOG(LogLevel::Warn, "Invalid property identifier in encode");
            break;
        }
    }
    ...
    bool Property::decode(ByteReader& r)
    {
        ...
        case ReasonString: {
            ...
            {
                REACTORMQ_LOG(reactormq::logging::LogLevel::Error, "Property", "Failed to decode string property");
                return false;
            }
            m_data = value;
            break;
        ...
        case UserProperty: {
            ...
            {
                REACTORMQ_LOG(reactormq::logging::LogLevel::Error, "Property", "Failed to decode user property");
                return false;
            }
        ...
        case Unknown:
            ...
            REACTORMQ_LOG(LogLevel::Warn, "Property", "Invalid property identifier in decode");
            return false;
        }
    ...
    uint32_t Property::getLength() const
    {
        ...
        case UserProperty: {
            const auto& [fst, snd] = std::get<std::pair<std::string, std::string>>(m_data);
            return static_cast<uint32_t>(sizeof(uint16_t) + fst.size() + sizeof(uint16_t) + snd.size() + 1); // +1 id
        }

        case Max:
        case Unknown: default: REACTORMQ_LOG(LogLevel::Warn, "Property", "Invalid property identifier in getLength");
            return 0;
        }
    }
    ...
    PropertyIdentifier Property::decodePropertyIdentifier(ByteReader& reader)
    {
        ...
        {
            REACTORMQ_LOG(LogLevel::Warn, "Property", "Invalid property identifier: %u", intCode);
