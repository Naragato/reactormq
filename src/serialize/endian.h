...
namespace reactormq::serialize
{
     ...
     */
    inline uint16_t hostToBigEndian16(const uint16_t v) { return kIsLittleEndian ? byteSwapUint16(v) : v; }
    /**
     * @brief Convert a 32-bit value from host to big-endian order.
     */
    inline uint32_t hostToBigEndian32(const uint32_t v) { return kIsLittleEndian ? byteSwapUint32(v) : v; }
    /**
     * @brief Convert a 64-bit value from host to big-endian order.
     */
    inline uint64_t hostToBigEndian64(const uint64_t v) { return kIsLittleEndian ? byteSwapUint64(v) : v; }
    /**
     * @brief Convert a 16-bit value from big-endian to host order.
     */
    inline uint16_t bigEndianToHost16(const uint16_t v) { return kIsLittleEndian ? byteSwapUint16(v) : v; }
    /**
     * @brief Convert a 32-bit value from big-endian to host order.
     */
    inline uint32_t bigEndianToHost32(const uint32_t v) { return kIsLittleEndian ? byteSwapUint32(v) : v; }
    /**
     * @brief Convert a 64-bit value from big-endian to host order.
     */
    inline uint64_t bigEndianToHost64(const uint64_t v) { return kIsLittleEndian ? byteSwapUint64(v) : v; }
}
