...
namespace reactormq::serialize
{
    ...
    public:
         ...
         */
        explicit ByteReader(const std::span<const std::byte>& buffer) : ByteReader(buffer.data(), buffer.size())
        {
        }
    ...
    private:
        ...
        bool tryReadBigEndian(T& out, const char* what)
        {
            ...
            {
                acc = (acc << 8) |
                      static_cast<uint64_t>(static_cast<uint8_t>(m_data[m_pos + i]));
            }
            m_pos += n;
        ...
        void logBounds(const char* what) const
        {
            REACTORMQ_LOG(
                ...
                "Serialize",
                "ByteReader: OOB while reading %s at %zu/%zu",
                what,
                m_pos,
                m_size);
        }
