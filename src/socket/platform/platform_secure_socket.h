...
namespace reactormq::socket
{
    ...
    public:
         ...
         */
        bool tryReceive(std::uint8_t* outData, const std::int32_t bufferSize, size_t& bytesRead) const override
        {
            ...
            const int result = SSL_read(m_ssl, outData, bufferSize);
            if (result > 0)
            {
                bytesRead = result;
                return true;
            ...
            // lets check for failed certificate verification
         ...
         * @return true if successful (may read 0 bytes if no data available), false on error.
         */
        bool tryReceiveWithFlags(
            std::uint8_t* outData, const std::int32_t bufferSize, const ReceiveFlags flags, size_t& bytesRead) const override
        {
            (void)flags;
        ...
        bool trySend(const std::uint8_t* data, const std::uint32_t size, size_t& bytesSent) const override
        {
            ...
            REACTORMQ_LOG(
                ...
                "PlatformSecureSocket: SSL_write failed: %s",
                getLastSslErrorString().c_str());
            bytesSent = 0;
            return false;
        }
         ...
         */
        [[nodiscard]] size_t getPendingData() const override
        {
    ...
    protected:
        ...
        static int32_t verifyCertificateCallback(const int32_t preverifyOk, X509_STORE_CTX* ctx)
        {
            ...
            const mqtt::SslVerifyCallback& customCallback = settings->getSslVerifyCallback();
            ...
            if (result == kPreverifySuccess)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Info,
                    "Certificate verification succeeded at depth %d: %s",
                    depth,
                    subjectName);
            }
            else
        ...
        bool initializeSslCommon()
        {
            ...
            if (nullptr == m_sslCtx)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Error,
                    "PlatformSecureSocket: SSL_CTX_new failed: %s",
                    getLastSslErrorString().c_str());
                return false;
            }
            ...
            if (m_settings && m_settings->shouldVerifyServerCertificate())
            {
                ...
                REACTORMQ_LOG(
                    logging::LogLevel::Info,
                    "PlatformSecureSocket: should verify certs");

                const TrustAnchorSet trustAnchors;
                if (!trustAnchors.empty())
                {
                    if (!trustAnchors.attachTo(m_sslCtx))
                    {
                        REACTORMQ_LOG(
                            logging::LogLevel::Warn,
                            "PlatformSecureSocket: failed to attach trust anchors to SSL_CTX");
                    }
                }
                else
                {
                    if (SSL_CTX_set_default_verify_paths(m_sslCtx) != 1)
                    {
                        REACTORMQ_LOG(
                            logging::LogLevel::Warn,
                            "PlatformSecureSocket: failed to set default verify paths");
                    }
                }
            ...
            if (nullptr == m_ssl)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Error,
                    "PlatformSecureSocket: SSL_new failed: %s",
                    getLastSslErrorString().c_str());
                cleanupSsl();
                return false;
            }
