...
namespace reactormq::socket
{
    ...
    public:
        ...
        void addAnchor(X509* cert)
        {
            ...
            if (notAfterTime != nullptr && X509_cmp_current_time(notAfterTime) < 0)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Warn,
                    "TrustAnchorSet: rejecting expired root certificate");
                X509_free(cert);
                return;
            }
            ...
            if (notBeforeTime != nullptr && X509_cmp_current_time(notBeforeTime) > 0)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Warn,
                    "TrustAnchorSet: rejecting not-yet-valid root certificate");
                X509_free(cert);
                return;
            }
            ...
            for (const X509* other: m_anchors)
            {
                if (X509_cmp(other, cert) == 0)
                {
                    REACTORMQ_LOG(
                        ...
                        "TrustAnchorSet: ignoring duplicate root certificate");
                    X509_free(cert);
                    return;
                }
            }
            ...
            REACTORMQ_LOG(
                logging::LogLevel::Debug,
                "TrustAnchorSet: adding new trust anchor");
            ...
            for (X509* cert: m_anchors)
            {
                ...
                if (X509_STORE_add_cert(store, cert) == 0)
                {
                    ...
                    if (ERR_GET_REASON(err) != X509_R_CERT_ALREADY_IN_HASH_TABLE)
                    {
                        REACTORMQ_LOG(
                            logging::LogLevel::Debug,
                            "TrustAnchorSet: X509_STORE_add_cert failed (0x%lx)",
                            err);
                    }
