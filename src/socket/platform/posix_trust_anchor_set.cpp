...
const char* envVarName = X509_get_default_cert_file_env();if (envVarName!= nullptr)
        {
...
const char* extraBundlePaths[] = { "/etc/pki/tls/certs/ca-bundle.crt", "/etc/ssl/certs/ca-certificates.crt", "/etc/ssl/ca-bundle.pem", };
for (const char* path : extraBundlePaths)
        {
            loadFromPemBundleFile(*this, path);
        }

const auto androidDir = "/system/etc/security/cacerts"; struct stat androidDirStat{};if (stat(androidDir, &androidDirStat) == 0 && S_ISDIR (
    androidDirStat.st_mode))
        {
