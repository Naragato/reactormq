...
return;
#else
CFArrayRef anchors = nullptr;if (const OSStatus status = SecTrustCopyAnchorCertificates(&anchors); status!= errSecSuccess|| anchors
== nullptr)
        {
            return;
        }
...
struct AnchorsReleaser
{
    ...
    // Rule of 3: we have a destructor, so explicitly handle copying.
    // This guard is not meant to be copied, so delete copy operations.
    AnchorsReleaser(const AnchorsReleaser&) = delete;

    AnchorsReleaser& operator=(const AnchorsReleaser&) = delete;

    // (Optional) allow move, if you ever need it
...
}; AnchorsReleaser nchorsReleaser{ anchors }; const CFIndex count = CFArrayGetCount(anchors);for (CFIndex index =0; index<count;++index)
        {
