...
namespace reactormq::socket
{
    bool PlatformSocket::createSocket()
    {
        ...
        size_t bAllow = 1; setsockopt(m_socket, SOL_SOCKET, SO_NOSIGPIPE, &bAllow, sizeof(bAllow));
    ...
    bool PlatformSocket::isConnected() const
    {
        ...
        if (getsockopt(m_socket, SOL_SOCKET, SO_ERROR, &soError, &optLen) != 0 || soError != 0)
        {
            if (soError != 0)
            {
    ...
    size_t PlatformSocket::getPendingData() const
    {
#if REACTORMQ_SOCKET_WITH_IOCTL
        if (size_t pendingBytes = 0; ioctlsocket(m_socket, FIONREAD, &pendingBytes) == 0)
        {
            return pendingBytes;
        }
#endif
    ...
    bool PlatformSocket::tryReceive(uint8_t* outData, const int32_t bufferSize, size_t& bytesRead) const
    {
        ...
        const ssize_t result = recv(m_socket, outData, static_cast<size_t>(bufferSize), 0);
    ...
    bool PlatformSocket::tryReceiveWithFlags(uint8_t* outData, const int32_t bufferSize, const ReceiveFlags flags, size_t& bytesRead) const
    {
        ...
        const ssize_t result = recv(m_socket, outData, static_cast<size_t>(bufferSize), platformFlags);
    ...
    bool PlatformSocket::trySend(const uint8_t* data, const uint32_t size, size_t& bytesSent) const
    {
        ...
        const int result = send(m_socket, data, size, 0);
        if (result >= 0)
        {
