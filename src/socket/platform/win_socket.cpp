...
namespace reactormq::socket
{
    bool PlatformSocket::createSocket()
    {
        ...
        REACTORMQ_LOG(logging::LogLevel::Debug, "PlatformSocket::createSocket() succeeded (handle=%p)", reinterpret_cast<void*>(m_socket));
    ...
    int PlatformSocket::connect(const std::string& host, const std::uint16_t port)
    {
        REACTORMQ_LOG(
            ...
            "PlatformSocket::connect() starting (host=%s, port=%u)",
            host.c_str(),
    ...
    void PlatformSocket::close()
    {
        ...
        REACTORMQ_LOG(logging::LogLevel::Debug, "PlatformSocket::close() closing socket (handle=%p)", reinterpret_cast<void*>(m_socket));
