...
namespace reactormq::socket
{
        ...
        : Socket(std::move(settings))
    {
        if (m_settings)
        {
            REACTORMQ_LOG(
                ...
                m_settings->getHost().c_str(),
                static_cast<unsigned>(m_settings->getPort()),
                m_settings->getClientId().c_str());
        }
        else
        {
...

        {
            if (m_socketPtr && m_socketPtr->isConnected())
            {
                REACTORMQ_LOG(
                    ...
                    m_settings ? m_settings->getHost().c_str() : "<null>",
                    m_settings ? static_cast<unsigned>(m_settings->getPort()) : 0U,
                    m_settings ? m_settings->getClientId().c_str() : "<null>");
                return;
            }
            ...
            const std::uint16_t port = m_settings->getPort();
...
#if REACTORMQ_SECURE_SOCKET_WITH_TLS || REACTORMQ_SECURE_SOCKET_WITH_BIO_TLS
            if (const mqtt::ConnectionProtocol protocol = m_settings->getProtocol(); protocol == mqtt::ConnectionProtocol::Tls || protocol
                == mqtt::ConnectionProtocol::Wss)
            {
                REACTORMQ_LOG(
                    logging::LogLevel::Debug,
                    "SecureSocket::connect() using PlatformSecureSocket (protocol=%d)",
                    static_cast<int>(protocol));
                m_socketPtr = std::make_unique<PlatformSecureSocket>(m_settings);
            }
            else
#endif
            {
    ...
    void SecureSocket::tick()
    {
...

        {
            if(nullptr == m_socketPtr)
            {
                // This is a very hot path; keep this log at Trace and minimal.
    ...
    bool SecureSocket::appendAndProcess(const uint8_t* data, const size_t size)
    {
        if (size == 0)
        {
...

        {
            ...
            capBytes = m_settings ? m_settings->getMaxBufferSize() : defaultCap;
    ...
    bool SecureSocket::readAvailableData()
    {
        ...
        const size_t pendingData = m_socketPtr->getPendingData();

        uint32_t chunkSize = 0;
        chunkSize = pendingData > 0 ? std::min(static_cast<uint32_t>(pendingData), kMaxChunkSize) : kMaxChunkSize;

        std::vector<uint8_t> tempBuffer(chunkSize);
        size_t bytesRead = 0;

        if (!m_socketPtr->tryReceive(tempBuffer.data(), static_cast<int32_t>(chunkSize), bytesRead))
        {
            if (const SocketError err = PlatformSocket::getLastError(); err == SocketError::WouldBlock)
            {
                return true;
        ...
        return appendAndProcess(tempBuffer.data(), bytesRead);
    }
} // namespace reactormq::socket