...
namespace reactormq::socket
{
     ...
     *
     */
    class Socket : public ISocketTickable
    {
    public:
        /**
         ...
         *
         */
        explicit Socket(mqtt::ConnectionSettingsPtr settings) : m_settings(std::move(settings))
        {
        }
         ...
         * @param size Number of bytes to send.
         */
        void send(const std::byte* data, const std::uint32_t size)
        {
            send(reinterpret_cast<const std::uint8_t*>(data), size);
        }
        ...
        bool readPacketsFromBuffer()
        {
            std::vector<std::vector<uint8_t>> packetsToDispatch;
            {
                const uint32_t maxPacketSize =
                    nullptr != m_settings ? m_settings->getMaxPacketSize() : 268435455u;

                size_t available = m_dataBuffer.size() - m_dataBufferReadOffset;

                constexpr uint8_t remainingLengthContinueBit = 0x80u;
                ...
                while (available > 1)
                {
                    ...
                    for (; index < 5; ++index)
                    {
                        ...
                        if ((encodedByte & remainingLengthContinueBit) == 0)
                        {
                ...
                if (m_dataBufferReadOffset > 0)
                {
                    ...
                    else
                    {
                        ...
                        const auto thresholdByFraction =
                            static_cast<size_t>(compactFraction * static_cast<float>(m_dataBuffer.size()));

                        if (m_dataBufferReadOffset >= compactMinBytes &&
                            m_dataBufferReadOffset >= thresholdByFraction)
                        {
                            m_dataBuffer.erase(
                                m_dataBuffer.begin(),
                                m_dataBuffer.begin() +
                                static_cast<std::ptrdiff_t>(m_dataBufferReadOffset));
                            m_dataBufferReadOffset = 0;
                        }
                    }
                }
