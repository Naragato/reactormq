...
namespace reactormq::logging
{
    ...
    std::string ConsoleSink::formatPrefix(const LogMessage& msg)
    {
        ...
        oss << toIso8601ms(msg.timestamp) << " | " << threadIdStr(msg.thread_id) << " | " << levelToString(msg.level) << " | " << msg.
            function << " | " << basename(msg.file) << ':' << msg.line << " | ";
        return oss.str();
    }
    ...
    void ConsoleSink::writeDuplicateSummary(const LogMessage& msg, const std::size_t count,
                                             ...
                                             const std::chrono::system_clock::time_point& lastTime) const
    {
        ...
        std::ostringstream oss;
        oss << firstTs << " to " << lastTs << " | " << tid << " | "
            << levelToString(msg.level) << " | " << msg.function << " | "
            << basename(msg.file) << ':' << msg.line << " | " << msg.text
    ...
    {
        return msg.text == last.text &&
               msg.function == last.function &&
               msg.file == last.file &&
               msg.line == last.line;
    }
} // namespace reactormq::logging