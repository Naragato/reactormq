...
namespace reactormq::logging
{
    ...
    void FileSink::writeMessage(const LogMessage& msg)
    {
        ...
        out_ << ts << " | " << tid << " | " << levelToString(msg.level) << " | "
             << msg.function << " | " << msg.file << ':' << msg.line << " | "
             << msg.text << '\n';
        out_.flush();
    }

    void FileSink::writeDuplicateSummary(const LogMessage& msg, const std::size_t count,
                                          ...
                                          const std::chrono::system_clock::time_point& lastTime)
    {
        ...
        out_ << firstTs << " to " << lastTs << " | " << tid << " | "
             << levelToString(msg.level) << " | " << msg.function << " | "
             << msg.file << ':' << msg.line << " | " << msg.text
